...
#Import data xlsx
library(rio)
gold <- import("https://github.com/shafifaris/MPDW/raw/main/Pertemuan%202/Gold%20Price.xlsx")
gold
```
...
```{r}
#Mengubah data agar terbaca sebagai time series
...
library(ggplot2)
plot(train_ma.ts, col="gold",main="Plot data latih")
points(train_ma.ts)
```

```{r}
#eksplorasi data uji
plot(test_ma.ts, col="gold",main="Plot data uji")
points(test_ma.ts)
```
...
Pemulusan menggunakan metode SMA dilakukan dengan fungsi `SMA()`. Dalam hal ini akan dilakukan pemulusan dengan parameter `n=4`.
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA sekitar 0.84%. Artinya, sangat akurat dalam memprediksi data latih. Kesalahan prediksinya sangat kecil, sehingga model ini dapat dipercaya untuk merepresentasikan pola historis pada data latih.
...
Nilai MAPE sebesar 0,82% menunjukkan bahwa rata-rata kesalahan prediksi model DMA (m=4) terhadap data latih hanya sekitar 0,82% dari nilai aktual. Dengan nilai yang jauh di bawah 10%, akurasi model dapat dikategorikan sangat baik. Hal ini berarti model DMA mampu merepresentasikan pola data latih dengan tingkat kesalahan yang sangat rendah sehingga layak digunakan sebagai dasar peramalan.
...
MAPE sebesar 11.18% menunjukkan bahwa rata-rata kesalahan prediksi model pada data uji adalah sekitar 11.18% dari nilai aktual. Nilai ini berada pada rentang 10–20%, sehingga model DMA (m=4) dapat dikategorikan memiliki akurasi yang cukup baik. Artinya, meskipun pada data uji model tidak seakurat pada data latih (MAPE jauh lebih kecil), hasil peramalan tetap cukup reliabel dan dapat digunakan sebagai acuan prediksi, meskipun perlu diwaspadai pada kondisi perubahan tren yang tajam.

Pada data latih, metode SMA sama baiknya jika dibandingkan dengan metode DMA. Sedangkan pada data uji, metode SMA lebih baik dibandingkan metode DMA.
...
akurasitesting_RMSE
```
Berdasarkan hasil perhitungan akurasi pada data uji, model SES dengan alpha 0.2 memberikan nilai SSE, MSE, dan RMSE terendah dibandingkan dengan model SES dengan alpha 0.7 dan optimum. Hal ini menunjukkan bahwa model dengan alpha 0.2 memiliki performa terbaik dalam memprediksi data uji, menghasilkan kesalahan prediksi yang paling kecil di antara ketiga model yang diuji.
...
# Kesimpulan 
Berdasarkan hasil uji akurasi pada data uji, model DES skenario 2 (λ=0,6 dan γ=0,3) memberikan nilai SSE, MSE, dan MAPE terendah dibandingkan dengan model SES dan skenario DES lainnya. Hal ini menunjukkan bahwa model DES skenario 2 memiliki performa terbaik dalam memprediksi data uji, menghasilkan kesalahan prediksi yang paling kecil di antara semua model diuji. Oleh karena itu, model DES dengan skenario 2 dapat dianggap sebagai pilihan terbaik untuk peramalan harga emas dalam konteks data ini.
