---
title: "Tugas Mandiri Kuliah Pertemuan 2" 
author: "Luthfia Aiman_G1401231064" 
...
tahun <- 2015:2024
ipm <- c(77.59, 78.38, 78.89, 79.53, 79.99, 79.97, 80.22, 80.64, 81.07, 81.55)
ipmdiy <- data.frame(tahun, ipm)
...
model_awal <- lm(ipm ~ tahun, data = ipmdiy)
summary(model_awal)
```
...
plot(ipmdiy$tahun, sisaan_awal, type="o", pch=20, col="red",
...
**Intuisi:** Metode ini secara iteratif (berulang-ulang) mencoba menemukan "tingkat penularan" error (disebut rho, ρ), lalu menyesuaikan model sampai autokorelasinya hilang. (Nanya-nanya pawon, konvergen)
...
```{r}
ipm.trans <- ipmdiy$ipm[-1] - ipmdiy$ipm[-12]*rho

tahun.trans <- ipmdiy$tahun[-1] - ipmdiy$tahun[-12]*rho
...
```{r}
library(HoRM)
model_hl <- hildreth.lu(ipmdiy$ipm, ipmdiy$tahun, rho = 0.411)
summary(model_hl)
```
...
-   Baik metode Cochrane-Orcutt maupun Hildreth-Lu berhasil menurunkan SSE model, artinya model yang baru lebih akurat (HL lebih akurat kerena SSE nya lebih kecil).
